...
import { dbClient, BillingComplaint, BillingComplaintType, CustomerEntry } from '@/lib/db';
import {
  Plus,
  Search,
  Trash2,
...
const BILLING_COMPLAINT_TYPE_LABELS: Record<BillingComplaintType, string> = {
  violation_report: 'Violation Report',
  usage_verification_of_consumer: 'Usage Verification of Consumer',
};

const BILLING_COMPLAINT_TYPE_COLORS: Record<BillingComplaintType, { bg: string; color: string; border: string }> = {
  violation_report: { bg: 'rgba(239,68,68,0.1)', color: '#f87171', border: 'rgba(239,68,68,0.2)' },
  usage_verification_of_consumer: { bg: 'rgba(14,165,233,0.1)', color: '#38bdf8', border: 'rgba(14,165,233,0.2)' },
};
...
export default function BillingComplaintsPage() {
  const { user } = useUser();
  ...
  const [categoryFilter, setCategoryFilter] = useState<BillingComplaintType | ''>('');

  // Modal state
  const [isInModalOpen, setIsInModalOpen] = useState(false);
  const [isOutModalOpen, setIsOutModalOpen] = useState(false);
  const [selectedComplaint, setSelectedComplaint] = useState<BillingComplaint | null>(null);
  const [editingComplaint, setEditingComplaint] = useState<BillingComplaint | null>(null);

  // IN Portion form state
  const [reference, setReference] = useState('');
  const [complaintDate, setComplaintDate] = useState('');
  const [complaintType, setComplaintType] = useState<BillingComplaintType>('violation_report');
  ...
  const fetchBillingComplaints = async () => {
    try {
      setLoading(true);
      const data = await dbClient.getBillingComplaints();
      setBillingComplaints(data);
    } catch (err) {
      console.error('Failed to fetch billing complaints', err);
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => { fetchBillingComplaints(); }, []);
  ...
  const handleOpenAddModal = () => {
    ...
    setComplaintType('violation_report');
    setCustomers([emptyCustomer()]);
    setIsInModalOpen(true);
  };

  const handleOpenEditModal = (c: BillingComplaint) => {
    setEditingComplaint(c);
    setReference(c.reference || '');
    setComplaintDate(c.complaint_date || '');
    setComplaintType((c.complaint_type as BillingComplaintType) || 'violation_report');
  ...
  const handleOpenReplyModal = (c: BillingComplaint) => {
  ...
  // --- Save handlers ---
  const handleSaveBillingComplaint = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      ...
      if (editingComplaint) {
        const { error } = await dbClient.updateBillingComplaint(editingComplaint.id, payload);
        if (error) alert(error);
        else { setIsInModalOpen(false); setEditingComplaint(null); fetchBillingComplaints(); }
      } else {
        const { error } = await dbClient.createBillingComplaint(payload);
        if (error) alert(error);
        else { setIsInModalOpen(false); fetchBillingComplaints(); }
      }
  ...
  const handleSaveReply = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await dbClient.updateBillingComplaint(selectedComplaint.id, {
      ...
      if (error) alert(error);
        ...
        setReplyText('');
        const updated = await dbClient.getBillingComplaints();
        setBillingComplaints(updated);
  ...
  const handleDeleteComplaint = async (id: string, createdBy: string) => {
    ...
    if (confirm('Are you sure you want to delete this billing complaint entry?')) {
      try {
        const { error } = await dbClient.deleteBillingComplaint(id);
        if (error) alert(error); else fetchBillingComplaints();
      } catch (err: any) { alert(err.message || 'Failed.'); }
    }
  };

  // --- Filter ---
  const filteredComplaints = billingComplaints.filter(c => {
    ...
    if (query) {
      ...
      matchesText = (
        ...
        normalizeText(BILLING_COMPLAINT_TYPE_LABELS[c.complaint_type as BillingComplaintType] || '').includes(query) ||
  ...
  const downloadCSV = () => {
    ...
    for (const c of filteredComplaints) {
      const typeLabel = BILLING_COMPLAINT_TYPE_LABELS[c.complaint_type as BillingComplaintType] || c.complaint_type;
    ...
    const categorySlug = categoryFilter ? BILLING_COMPLAINT_TYPE_LABELS[categoryFilter].toLowerCase().replace(/\s+/g, '_') : 'all';
    link.setAttribute('download', `billing_complaints_${categorySlug}_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1 className="page-title">Billing Complaints Registry</h1>
          <p className="page-description">Manage billing complaints — violation reports and consumer usage verifications</p>
        </div>
        {user?.role !== 'lawyer' && (
          <button onClick={handleOpenAddModal} className="btn btn-primary">
            <Plus className="w-4 h-4" /> Add Billing Complaint
          </button>
        )}
      </div>
      ...
      <div className="glass-panel filter-bar" style={{ padding: '16px', marginBottom: '32px', display: 'flex', gap: '16px', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          ...
          <select
            ...
            onChange={e => setCategoryFilter(e.target.value as BillingComplaintType | '')}
          >
            <option value="">All Types</option>
            <option value="violation_report">Violation Report</option>
            <option value="usage_verification_of_consumer">Usage Verification of Consumer</option>
          </select>
          {categoryFilter && (
            <button
              type="button"
      ...
      <div className="glass-panel" style={{ padding: '0', overflow: 'hidden' }}>
        {loading ? (
          ...
          <div style={{ padding: '80px 40px', textAlign: 'center', color: 'var(--text-secondary)' }}>
            ...
            <h3>No Billing Complaints Found</h3>
            ...
            <table className="data-table">
              ...
              <tbody>
                {filteredComplaints.map(c => {
                  ...
                  const typeColors = BILLING_COMPLAINT_TYPE_COLORS[c.complaint_type as BillingComplaintType] || BILLING_COMPLAINT_TYPE_COLORS.violation_report;
                  const typeLabel = BILLING_COMPLAINT_TYPE_LABELS[c.complaint_type as BillingComplaintType] || c.complaint_type;
                  const custs = c.customers || [];
        ...
        <div className="modal-overlay animate-fade-in">
          <div className="glass-panel modal-content" style={{ maxWidth: '700px', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div className="modal-header">
              <h2>{editingComplaint ? 'Edit Billing Complaint' : 'Register Billing Complaint (In Portion)'}</h2>
              <p style={{ color: 'var(--text-secondary)', fontSize: '13px', marginTop: '4px' }}>
                Fill in the memorandum reference, type, and customer details.
              </p>
            </div>

            <form onSubmit={handleSaveBillingComplaint}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginBottom: '24px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                  <div className="form-group">
                    ...
                    <input type="text" className="form-input" placeholder="e.g. BIL/VR-2026" value={reference} onChange={e => setReference(e.target.value)} required />
                  </div>
                  <div className="form-group">
                ...
                {/* Complaint Type */}
                <div className="form-group">
                  <label className="form-label">Billing Complaint Type</label>
                  <select className="form-select" value={complaintType} onChange={e => setComplaintType(e.target.value as BillingComplaintType)} required>
                    <option value="violation_report">Violation Report</option>
                    <option value="usage_verification_of_consumer">Usage Verification of Consumer</option>
                  </select>
                </div>
                      ...
                      <div key={idx} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border-color)', borderRadius: '8px', padding: '14px', position: 'relative' }}>
                        ...
                        <div className="form-group" style={{ marginBottom: '0' }}>
                          ...
                          <input
                            type="text"
                            className="form-input"
                            placeholder="e.g. Excessive billing mismatch, suspected tampering, meter slow..."
                            value={cu.anomalies || ''}
          ...
          <div className="glass-panel modal-content" style={{ maxWidth: '680px', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <form onSubmit={handleSaveReply}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginBottom: '24px' }}>
                ...
                <div>
                  {(() => {
                    const ct = selectedComplaint.complaint_type as BillingComplaintType;
                    const tc = BILLING_COMPLAINT_TYPE_COLORS[ct] || BILLING_COMPLAINT_TYPE_COLORS.violation_report;
                    return <span style={{ display: 'inline-block', fontSize: '11px', fontWeight: '600', padding: '3px 10px', borderRadius: '5px', background: tc.bg, color: tc.color, border: `1px solid ${tc.border}` }}>{BILLING_COMPLAINT_TYPE_LABELS[ct] || ct}</span>;
                  })()}
                </div>
                ...
                {user?.role !== 'lawyer' && (
                  <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '16px' }}>
                    ...
                    <div className="form-group">
                      ...
                      <textarea
                        className="form-textarea"
                        placeholder="Describe the billing action taken for this consumer (e.g. Meter tested and verified, bill revised, fine imposed...)"
                        rows={3}
                        value={replyText}
